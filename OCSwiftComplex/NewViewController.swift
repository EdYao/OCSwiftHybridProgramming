...
class NewViewController: UIViewController {
    ...
    @IBAction func changeTitleBtnClicked(_ sender: Any) {
        if self.checkTitleLegal(title: textField.text!) {//check the content that user inputted legal
            self.parentVC.changeTitle(self.textField.text);
            self.navigationController?.popViewController(animated: true)
        }
    }
    
    func checkTitleLegal(title : String) -> Bool {
        if title.isEmpty {
            let alertView = UIAlertController.init(title: "Error", message: "Please enter the title", preferredStyle: UIAlertControllerStyle(rawValue: 1)!);
